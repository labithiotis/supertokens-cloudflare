 ...
 * under the License.
 */
import { normaliseInputAppInfoOrThrowError, maxVersion, normaliseHttpMethod, sendNon200ResponseWithMessage, getRidFromHeader, } from "./utils";
...
export default class SuperTokens {
    constructor(config) {
        ...
        this.getUserCount = async (includeRecipeIds, tenantId, userContext) => {
            ...
            let response = await querier.sendGetRequest(new NormalisedURLPath(`/${tenantId === undefined ? DEFAULT_TENANT_ID : tenantId}/users/count`), {
        ...
        this.middleware = async (request, response, userContext) => {
            ...
            if (!path.startsWith(this.appInfo.apiBasePath)) {
                logDebugMessage("middleware: Not handling because request path did not start with config path. Request path: " +
            ...
            async function handleWithoutRid(recipeModules) {
                for (let i = 0; i < recipeModules.length; i++) {
                    logDebugMessage("middleware: Checking recipe ID for match: " +
                    ...
                    if (idResult !== undefined) {
                        ...
                        let requestHandled = await recipeModules[i].handleAPIRequest(idResult.id, idResult.tenantId, request, response, path, method, userContext);
                        if (!requestHandled) {
        ...
        Querier.init((_a = config.supertokens) === null || _a === void 0 ? void 0 : _a.connectionURI.split(";").filter((h) => h !== "").map((h) => {
            return {
