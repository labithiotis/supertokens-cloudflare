...
export const AuthUtils = {
     ...
     */
    preAuthChecks: async function ({ authenticatingAccountInfo, tenantId, isSignUp, isVerified, signInVerifiesLoginMethod, authenticatingUser, factorIds, skipSessionUserUpdateInCore, session, userContext, }) {
        let validFactorIds;
        ...
        const authTypeInfo = await AuthUtils.checkAuthTypeAndLinkingStatus(session, authenticatingAccountInfo, authenticatingUser, skipSessionUserUpdateInCore, userContext);
        ...
        if (authTypeInfo.isFirstFactor) {
            ...
            const validFirstFactors = await AuthUtils.filterOutInvalidFirstFactorsOrThrowIfAllAreInvalid(factorIds, tenantId, session !== undefined, userContext);
    ...
    postAuthChecks: async function ({ authenticatedUser, recipeUserId, isSignUp, factorId, session, req, res, tenantId, userContext, }) {
        logDebugMessage(`postAuthChecks called ${session !== undefined ? "with" : "without"} a session to ${isSignUp ? "sign in" : "sign up"} with ${factorId}`);
    ...
    getAuthenticatingUserAndAddToCurrentTenantIfRequired: async ({ recipeId, accountInfo, checkCredentialsOnTenant, tenantId, session, userContext, }) => {
        let i = 0;
        while (i++ < 300) {
            logDebugMessage(`getAuthenticatingUserAndAddToCurrentTenantIfRequired called with ${JSON.stringify(accountInfo)}`);
            ...
            logDebugMessage(`getAuthenticatingUserAndAddToCurrentTenantIfRequired got ${existingUsers.length} users from the core resp`);
            ...
            logDebugMessage(`getAuthenticatingUserAndAddToCurrentTenantIfRequired got ${usersWithMatchingLoginMethods.length} users with matching login methods`);
            ...
            if (authenticatingUser === undefined && session !== undefined) {
                ...
                if (!sessionUser.isPrimaryUser) {
                    logDebugMessage(`getAuthenticatingUserAndAddToCurrentTenantIfRequired session user is non-primary so returning early without checking other tenants`);
                    return undefined;
                }
                const matchingLoginMethodsFromSessionUser = sessionUser.loginMethods.filter((lm) => lm.recipeId === recipeId &&
                    (lm.hasSameEmailAs(accountInfo.email) ||
                        lm.hasSamePhoneNumberAs(accountInfo.phoneNumber) ||
                        lm.hasSameThirdPartyInfoAs(accountInfo.thirdParty)));
                logDebugMessage(`getAuthenticatingUserAndAddToCurrentTenantIfRequired session has ${matchingLoginMethodsFromSessionUser.length} matching login methods`);
                ...
                for (const lm of matchingLoginMethodsFromSessionUser) {
                    logDebugMessage(`getAuthenticatingUserAndAddToCurrentTenantIfRequired session checking credentials on ${lm.tenantIds[0]}`);
                    if (await checkCredentialsOnTenant(lm.tenantIds[0])) {
                        logDebugMessage(`getAuthenticatingUserAndAddToCurrentTenantIfRequired associating user from ${lm.tenantIds[0]} with current tenant`);
                        const associateRes = await MultiTenancy.associateUserToTenant(tenantId, lm.recipeUserId, userContext);
                        logDebugMessage(`getAuthenticatingUserAndAddToCurrentTenantIfRequired associating returned ${associateRes.status}`);
    ...
    checkAuthTypeAndLinkingStatus: async function (session, accountInfo, inputUser, skipSessionUserUpdateInCore, userContext) {
        ...
        if (session === undefined) {
            ...
            if (inputUser !== undefined && inputUser.id === session.getUserId()) {
                logDebugMessage(`checkAuthTypeAndLinkingStatus returning secondary factor, session and input user are the same`);
            ...
            logDebugMessage(`checkAuthTypeAndLinkingStatus loading session user, ${inputUser === null || inputUser === void 0 ? void 0 : inputUser.id} === ${session.getUserId()}`);
            // We have to load the session user in order to get the account linking info
            const sessionUserResult = await AuthUtils.tryAndMakeSessionUserIntoAPrimaryUser(session, skipSessionUserUpdateInCore, userContext);
            ...
            const shouldLink = await AccountLinking.getInstance().config.shouldDoAutomaticAccountLinking(accountInfo, sessionUser, session, session.getTenantId(), userContext);
            logDebugMessage(`checkAuthTypeAndLinkingStatus session user <-> input user shouldDoAutomaticAccountLinking returned ${JSON.stringify(shouldLink)}`);
     ...
     * if the session user should be primary but we couldn't make it primary because of a conflicting primary user.
     */
    linkToSessionIfProvidedElseCreatePrimaryUserIdOrLinkByAccountInfo: async function ({ tenantId, inputUser, recipeUserId, session, userContext, }) {
        ...
        const authTypeRes = await AuthUtils.checkAuthTypeAndLinkingStatus(session, authLoginMethod, inputUser, false, userContext);
        if (authTypeRes.status !== "OK") {
            return authTypeRes;
        }
        if (authTypeRes.isFirstFactor) {
            if (!recipeInitDefinedShouldDoAutomaticAccountLinking(AccountLinking.getInstance().config)) {
                logDebugMessage("linkToSessionIfProvidedElseCreatePrimaryUserIdOrLinkByAccountInfo skipping link by account info because this is a first factor auth and the app hasn't defined shouldDoAutomaticAccountLinking");
                return { status: "OK", user: inputUser };
            }
            logDebugMessage("linkToSessionIfProvidedElseCreatePrimaryUserIdOrLinkByAccountInfo trying to link by account info because this is a first factor auth");
        ...
        logDebugMessage("linkToSessionIfProvidedElseCreatePrimaryUserIdOrLinkByAccountInfo trying to link by session info");
    ...
    tryAndMakeSessionUserIntoAPrimaryUser: async function (session, skipSessionUserUpdateInCore, userContext) {
        ...
        if (sessionUser.isPrimaryUser) {
            ...
            const shouldDoAccountLinking = await AccountLinking.getInstance().config.shouldDoAutomaticAccountLinking(sessionUser.loginMethods[0], undefined, session, session.getTenantId(userContext), userContext);
            logDebugMessage(`tryAndMakeSessionUserIntoAPrimaryUser shouldDoAccountLinking: ${JSON.stringify(shouldDoAccountLinking)}`);
            if (shouldDoAccountLinking.shouldAutomaticallyLink) {
                ...
                if (shouldDoAccountLinking.shouldRequireVerification && !sessionUser.loginMethods[0].verified) {
                    ...
                    if ((await session.getClaimValue(EmailVerificationClaim, userContext)) !== false) {
                        logDebugMessage(`tryAndMakeSessionUserIntoAPrimaryUser updating emailverification status in session`);
                ...
                logDebugMessage(`tryAndMakeSessionUserIntoAPrimaryUser createPrimaryUser returned ${createPrimaryUserRes.status}`);
    ...
    tryLinkingBySession: async function ({ linkingToSessionUserRequiresVerification, authLoginMethod, authenticatedUser, sessionUser, userContext, }) {
        ...
        if (linkAccountsResult.status === "OK") {
            ...
            logDebugMessage("tryLinkingBySession linking to session user failed because of a race condition - input user linked to another user");
            ...
            logDebugMessage("tryLinkingBySession linking to session user failed because of a race condition - INPUT_USER_IS_NOT_A_PRIMARY_USER, should retry");
            ...
            logDebugMessage("tryLinkingBySession linking to session user failed because of a race condition - input user has another primary user it can be linked to");
...
async function filterOutInvalidSecondFactorsOrThrowIfAllAreInvalid(factorIds, inputUserAlreadyLinkedToSessionUser, sessionUser, session, userContext) {
    ...
    if (mfaInstance !== undefined) {
        if (!inputUserAlreadyLinkedToSessionUser) {
            ...
            logDebugMessage(`filterOutInvalidSecondFactorsOrThrowIfAllAreInvalid checking if linking is allowed by the mfa recipe`);
            ...
            // then we do not want to include a link in the email.
            for (const id of factorIds) {
                logDebugMessage(`filterOutInvalidSecondFactorsOrThrowIfAllAreInvalid checking assertAllowedToSetupFactorElseThrowInvalidClaimError`);
                try {
                    ...
                    logDebugMessage(`filterOutInvalidSecondFactorsOrThrowIfAllAreInvalid ${id} valid because assertAllowedToSetupFactorElseThrowInvalidClaimError passed`);
                    // we add it to the valid factor ids list since it is either already set up or allowed to be set up
                    validFactorIds.push(id);
                }
                catch (err) {
                    logDebugMessage(`filterOutInvalidSecondFactorsOrThrowIfAllAreInvalid assertAllowedToSetupFactorElseThrowInvalidClaimError failed for ${id}`);
                    caughtSetupFactorError = err;
                }
            }
            if (validFactorIds.length === 0) {
                logDebugMessage(`filterOutInvalidSecondFactorsOrThrowIfAllAreInvalid rethrowing error from assertAllowedToSetupFactorElseThrowInvalidClaimError because we found no valid factors`);
            ...
            logDebugMessage(`filterOutInvalidSecondFactorsOrThrowIfAllAreInvalid allowing all factors because it'll not create new link`);
            return factorIds;
        }
    }
    else {
        logDebugMessage(`filterOutInvalidSecondFactorsOrThrowIfAllAreInvalid allowing all factors because MFA is not enabled`);
