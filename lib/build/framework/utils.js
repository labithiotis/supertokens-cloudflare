...
export function getCookieValueFromHeaders(headers, key) {
...
export function getCookieValueFromIncomingMessage(request, key) {
    if (request.cookies) {
        return request.cookies[key];
    }
    return getCookieValueFromHeaders(request.headers, key);
}
export function getHeaderValueFromIncomingMessage(request, key) {
...
export async function parseJSONBodyFromRequest(req) {
...
export async function parseURLEncodedFormData(req) {
    const encoding = getCharset(req) || "utf-8";
    if (!encoding.startsWith("utf-")) {
        throw new Error(`unsupported charset ${encoding.toUpperCase()}`);
    }
...
export async function assertThatBodyParserHasBeenUsedForExpressLikeRequest(method, request) {
...
export async function assertFormDataBodyParserHasBeenUsedForExpressLikeRequest(request) {
    if (typeof request.body === "string") {
        try {
...
export function setHeaderForExpressLikeResponse(res, key, value, allowDuplicateKey) {
    var _a, _b;
    try {
...
*/
        ...
        throw new Error("Error while setting header with key: " +
            ...
            value +
            "\nError: " +
            ((_b = (_a = err) === null || _a === void 0 ? void 0 : _a.message) !== null && _b !== void 0 ? _b : err));
    }
}
/**
 *
 ...
 * @param path
 */
export function setCookieForServerResponse(res, key, value, domain, secure, httpOnly, expires, path, sameSite) {
    return appendToServerResponse(res, COOKIE_HEADER, serializeCookieValue(key, value, domain, secure, httpOnly, expires, path, sameSite), key);
}
/**
...
export function getCookieValueToSetInHeader(prev, val, key) {
    let value = val;
    if (prev !== undefined) {
...
export function serializeCookieValue(key, value, domain, secure, httpOnly, expires, path, sameSite) {
    let opts = {
        domain,
        secure,
        httpOnly,
