...
class NextJS {
    ...
    static getAppDirRequestHandler(NextResponse) {
        ...
        return async function handleCall(req) {
            ...
            for (const respCookie of baseResponse.cookies) {
                baseResponse.headers.append("Set-Cookie", (0, cookie_1.serialize)(respCookie.key, respCookie.value, {
    ...
    static async commonSSRSession(baseRequest, options, userContext) {
        ...
        const hasToken = transferMethods.some((transferMethod) => {
            const token = (0, cookieAndHeaders_1.getToken)(baseRequest, "access", transferMethod);
            if (!token) {
                return false;
            }
            try {
                (0, jwt_1.parseJWTWithoutSignatureVerification)(token);
                return true;
            }
            catch (_a) {
                return false;
    ...
    static async getSSRSession(cookies, headers, options, userContext) {
        ...
        const _a = await NextJS.commonSSRSession(baseRequest, options, (0, utils_1.getUserContext)(userContext)), { baseResponse, nextResponse } = _a, result = __rest(_a, ["baseResponse", "nextResponse"]);
        return result;
    }
    static async withSession(req, handler, options, userContext) {
        try {
            ...
            const { session, nextResponse, baseResponse } = await NextJS.commonSSRSession(baseRequest, options, (0, utils_1.getUserContext)(userContext));
            if (nextResponse) {
                return nextResponse;
            }
            let userResponse;
