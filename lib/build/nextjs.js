...
import { CollectingResponse, PreParsedRequest, middleware, errorHandler as customErrorHandler, } from "./framework/custom";
...
export default class NextJS {
    ...
    static getAppDirRequestHandler(NextResponse) {
        ...
        return async function handleCall(req) {
            ...
            for (const respCookie of baseResponse.cookies) {
                baseResponse.headers.append("Set-Cookie", serialize(respCookie.key, respCookie.value, {
    ...
    static async getSSRSession(cookies, headers, options, userContext) {
        ...
        const _a = await NextJS.commonSSRSession(baseRequest, options, getUserContext(userContext)), { baseResponse, nextResponse } = _a, result = __rest(_a, ["baseResponse", "nextResponse"]);
        return result;
    }
    static async withSession(req, handler, options, userContext) {
        try {
            ...
            const { session, nextResponse, baseResponse } = await NextJS.commonSSRSession(baseRequest, options, getUserContext(userContext));
            if (nextResponse) {
                return nextResponse;
            }
            let userResponse;
            ...
            for (const respCookie of baseResponse.cookies) {
                didAddCookies = true;
                userResponse.headers.append("Set-Cookie", serialize(respCookie.key, respCookie.value, {
                    domain: respCookie.domain,
                    expires: new Date(respCookie.expires),
                    httpOnly: respCookie.httpOnly,
                    path: respCookie.path,
