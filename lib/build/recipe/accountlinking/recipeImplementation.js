...
export default function getRecipeImplementation(querier, config, recipeInstance) {
    return {
        getUsers: async function ({ tenantId, timeJoinedOrder, limit, paginationToken, includeRecipeIds, query, userContext, }) {
            ...
            let response = await querier.sendGetRequest(new NormalisedURLPath(`${tenantId !== null && tenantId !== void 0 ? tenantId : "public"}/users`), Object.assign({ includeRecipeIds: includeRecipeIdsStr, timeJoinedOrder: timeJoinedOrder, limit: limit, paginationToken: paginationToken }, query), userContext);
            return {
        ...
        linkAccounts: async function ({ recipeUserId, primaryUserId, userContext, }) {
            const accountsLinkingResult = await querier.sendPostRequest(new NormalisedURLPath("/recipe/accountlinking/user/link"), {
                recipeUserId: recipeUserId.getAsString(),
                primaryUserId,
            }, userContext);
        ...
        unlinkAccount: async function ({ recipeUserId, userContext, }) {
            let accountsUnlinkingResult = await querier.sendPostRequest(new NormalisedURLPath("/recipe/accountlinking/user/unlink"), {
                recipeUserId: recipeUserId.getAsString(),
            }, userContext);
        ...
        listUsersByAccountInfo: async function ({ tenantId, accountInfo, doUnionOfAccountInfo, userContext, }) {
            var _a, _b;
            let result = await querier.sendGetRequest(new NormalisedURLPath(`${tenantId !== null && tenantId !== void 0 ? tenantId : "public"}/users/by-accountinfo`), {
