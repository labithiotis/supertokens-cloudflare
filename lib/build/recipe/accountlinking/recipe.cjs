...
const _Recipe = class _Recipe extends import_recipeModule.default {
  constructor(recipeId, appInfo, config, _recipes, _ingredients) {
    ...
    this.getPrimaryUserThatCanBeLinkedToRecipeUserId = async ({
      ...
      (0, import_logger.logDebugMessage)(`getPrimaryUserThatCanBeLinkedToRecipeUserId found ${users.length} matching users`);
      let pUsers = users.filter((u) => u.isPrimaryUser);
      (0, import_logger.logDebugMessage)(`getPrimaryUserThatCanBeLinkedToRecipeUserId found ${pUsers.length} matching primary users`);
      if (pUsers.length > 1) {
    ...
    this.getOldestUserThatCanBeLinkedToRecipeUser = async ({
      ...
      (0, import_logger.logDebugMessage)(`getOldestUserThatCanBeLinkedToRecipeUser found ${users.length} matching users`);
    ...
    this.isSignInAllowed = async ({
      ...
      import_processState.ProcessState.getInstance().addState(import_processState.PROCESS_STATE.IS_SIGN_IN_ALLOWED_CALLED);
    ...
    this.isSignUpAllowed = async ({
      ...
      import_processState.ProcessState.getInstance().addState(import_processState.PROCESS_STATE.IS_SIGN_UP_ALLOWED_CALLED);
    ...
    this.isSignInUpAllowedHelper = async ({
      ...
      import_processState.ProcessState.getInstance().addState(import_processState.PROCESS_STATE.IS_SIGN_IN_UP_ALLOWED_HELPER_CALLED);
      let users = await this.recipeInterfaceImpl.listUsersByAccountInfo({
        tenantId,
        accountInfo,
        doUnionOfAccountInfo: true,
      ...
      if (users.length === 0) {
        (0, import_logger.logDebugMessage)("isSignInUpAllowedHelper returning true because no user with given account info");
        return true;
      }
      ...
      if (primaryUsers.length === 0) {
        (0, import_logger.logDebugMessage)("isSignInUpAllowedHelper no primary user exists");
        ...
        if (!shouldDoAccountLinking.shouldAutomaticallyLink) {
          (0, import_logger.logDebugMessage)("isSignInUpAllowedHelper returning true because account linking is disabled");
          return true;
        }
        ...
        for (let i = 0; i < users.length; i++) {
          ...
          if (accountInfo.email !== void 0) {
            if (currUser.loginMethods[0].hasSameEmailAs(accountInfo.email) && currUser.loginMethods[0].verified) {
              (0, import_logger.logDebugMessage)("isSignInUpAllowedHelper found same email for another user and verified");
        ...
        import_processState.ProcessState.getInstance().addState(import_processState.PROCESS_STATE.IS_SIGN_IN_UP_ALLOWED_NO_PRIMARY_USER_EXISTS);
        (0, import_logger.logDebugMessage)("isSignInUpAllowedHelper returning " + shouldAllow);
        return shouldAllow;
      } else {
        ...
        (0, import_logger.logDebugMessage)("isSignInUpAllowedHelper primary user found");
        let shouldDoAccountLinking = await this.config.shouldDoAutomaticAccountLinking(
          accountInfo,
          primaryUser,
          session,
    ...
    this.isEmailChangeAllowed = async (input) => {
      ...
      for (const tenantId of inputUser.tenantIds) {
        ...
        if (inputUser.isPrimaryUser) {
          ...
          if (!shouldDoAccountLinking.shouldAutomaticallyLink) {
            (0, import_logger.logDebugMessage)(`isEmailChangeAllowed: can change on ${tenantId} cause linking is disabled`);
            continue;
          }
          if (!shouldDoAccountLinking.shouldRequireVerification) {
            (0, import_logger.logDebugMessage)(
      ...
      if (input.user.isPrimaryUser) {
        ...
        if (recipeUserEmail !== void 0) {
          ...
          if (shouldVerifyEmail) {
            let resp = await import_recipe.default.getInstanceOrThrowError().recipeInterfaceImpl.createEmailVerificationToken(
              {
            ...
            if (resp.status === "OK") {
              await import_recipe.default.getInstanceOrThrowError().recipeInterfaceImpl.verifyEmailUsingToken(
                {
    ...
    this.config = (0, import_utils.validateAndNormaliseUserInput)(appInfo, config);
    {
      let builder = new import_supertokens_js_override.default(
        (0, import_recipeImplementation.default)(import_querier.Querier.getNewInstanceOrThrowError(recipeId), this.config, this)
      );
  ...
  static getInstance() {
    if (_Recipe.instance === void 0) {
      _Recipe.init()(
        import_supertokens.default.getInstanceOrThrowError().appInfo,
        import_supertokens.default.getInstanceOrThrowError().isInServerlessEnv
      );
    }
    return _Recipe.instance;
  }
  ...
  isErrorFromThisRecipe(err) {
    return import_error2.default.isErrorFromSuperTokens(err) && err.fromRecipe === _Recipe.RECIPE_ID;
  }
  static reset() {
  ...
  async shouldBecomePrimaryUser(user, tenantId, session, userContext) {
    ...
    if (!shouldDoAccountLinking.shouldAutomaticallyLink) {
      (0, import_logger.logDebugMessage)("shouldBecomePrimaryUser returning false because shouldAutomaticallyLink is false");
      return false;
    }
    ...
    (0, import_logger.logDebugMessage)("shouldBecomePrimaryUser returning true");
    return true;
  }
  async tryLinkingByAccountInfoOrCreatePrimaryUser({
    ...
    while (tries++ < 100) {
      ...
      if (primaryUserThatCanBeLinkedToTheInputUser !== void 0) {
        (0, import_logger.logDebugMessage)("tryLinkingByAccountInfoOrCreatePrimaryUser: got primary user we can try linking");
        if (!primaryUserThatCanBeLinkedToTheInputUser.loginMethods.some(
          ...
          (0, import_logger.logDebugMessage)("tryLinkingByAccountInfoOrCreatePrimaryUser linking");
          ...
          if (linkAccountsResult.status === "OK") {
            (0, import_logger.logDebugMessage)("tryLinkingByAccountInfoOrCreatePrimaryUser successfully linked");
            ...
            (0, import_logger.logDebugMessage)("tryLinkingByAccountInfoOrCreatePrimaryUser already linked to another user");
            return {
              status: "OK",
      ...
      if (oldestUserThatCanBeLinkedToTheInputUser !== void 0 && oldestUserThatCanBeLinkedToTheInputUser.id !== inputUser.id) {
        (0, import_logger.logDebugMessage)("tryLinkingByAccountInfoOrCreatePrimaryUser: got an older user we can try linking");
      ...
      (0, import_logger.logDebugMessage)("tryLinkingByAccountInfoOrCreatePrimaryUser: trying to make the current user primary");
