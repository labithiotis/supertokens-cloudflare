...
class Wrapper {
  static async createJWT(payload, validitySeconds, useStaticSigningKey, userContext) {
    return await import_recipe.default.getInstanceOrThrowError().recipeInterfaceImpl.createJWT({
      payload,
      validitySeconds,
      useStaticSigningKey,
  ...
  static async getJWKS(userContext) {
    return await import_recipe.default.getInstanceOrThrowError().recipeInterfaceImpl.getJWKS({
      userContext: (0, import_utils.getUserContext)(userContext)
    });
  }
}
