...
export default async function emailVerify(apiImplementation, tenantId, options, userContext) {
    let result;
    if (normaliseHttpMethod(options.req.getMethod()) === "post") {
        ...
        const session = await Session.getSession(options.req, options.res, { overrideGlobalClaimValidators: () => [], sessionRequired: false }, userContext);
        ...
        const session = await Session.getSession(options.req, options.res, { overrideGlobalClaimValidators: () => [] }, userContext);
