...
export default function getAPIInterface() {
    return {
        ...
        isEmailVerifiedGET: async function ({ userContext, session, options }) {
            ...
            const emailInfo = await EmailVerificationRecipe.getInstanceOrThrowError().getEmailForRecipeUserId(undefined, session.getRecipeUserId(userContext), userContext);
        ...
        generateEmailVerifyTokenPOST: async function ({ options, userContext, session }) {
            ...
            if (emailInfo.status === "EMAIL_DOES_NOT_EXIST_ERROR") {
                logDebugMessage(`Email verification email not sent to user ${session
                ...
                // or if it belong to a recipe user ID that got deleted. Either way,
                // we logout the user.
                logDebugMessage("generateEmailVerifyTokenPOST: Returning UNAUTHORISED because the user id provided is unknown");
