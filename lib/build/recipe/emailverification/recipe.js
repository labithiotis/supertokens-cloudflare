...
export default class Recipe extends RecipeModule {
    constructor(recipeId, appInfo, isInServerlessEnv, config, ingredients) {
        ...
        this.updateSessionIfRequiredPostEmailVerification = async (input) => {
            ...
            if (input.session !== undefined) {
                ...
                if (input.session.getRecipeUserId(input.userContext).getAsString() ===
                    ...
                    logDebugMessage("updateSessionIfRequiredPostEmailVerification the session belongs to the verified user");
                    ...
                    if (input.session.getUserId() === primaryUserId) {
                        logDebugMessage("updateSessionIfRequiredPostEmailVerification the session userId matches the primary user id, so we are only refreshing the claim");
                        ...
                        logDebugMessage("updateSessionIfRequiredPostEmailVerification the session user id doesn't match the primary user id, so we are revoking all sessions and creating a new one");
                        ...
                        await Session.revokeAllSessionsForUser(input.recipeUserIdWhoseEmailGotVerified.getAsString(), false, input.session.getTenantId(), input.userContext);
                        // create a new session and return that..
                        return await Session.createNewSession(input.req, input.res, input.session.getTenantId(), input.session.getRecipeUserId(input.userContext), {}, {}, input.userContext);
                    }
                }
                else {
                    logDebugMessage("updateSessionIfRequiredPostEmailVerification the verified user doesn't match the session");
        ...
        {
            let builder = new OverrideableBuilder(RecipeImplementation(Querier.getNewInstanceOrThrowError(recipeId), this.getEmailForRecipeUserId));
            ...
            if (Recipe.instance === undefined) {
                ...
                PostSuperTokensInitCallbacks.addPostInitCallback(() => {
                    ...
                    if (config.mode === "REQUIRED") {
                        SessionRecipe.getInstanceOrThrowError().addClaimValidatorFromOtherRecipe(EmailVerificationClaim.validators.isVerified());
                    }
                });
                return Recipe.instance;
            }
