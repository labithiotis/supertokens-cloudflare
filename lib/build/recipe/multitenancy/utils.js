...
export const isValidFirstFactor = async function (tenantId, factorId, userContext) {
    var _a;
    ...
    logDebugMessage(`isValidFirstFactor got ${(_a = tenantConfig.firstFactors) === null || _a === void 0 ? void 0 : _a.join(", ")} from tenant config`);
    logDebugMessage(`isValidFirstFactor got ${firstFactorsFromMFA} from MFA`);
    logDebugMessage(`isValidFirstFactor tenantconfig enables: ${Object.keys(tenantConfig).filter((k) => { var _a; return (_a = tenantConfig[k]) === null || _a === void 0 ? void 0 : _a.enabled; })}`);
...
export function isFactorConfiguredForTenant({ allAvailableFirstFactors, firstFactors, factorId, }) {
    ...
    let configuredFirstFactors = firstFactors.filter((factorId) => allAvailableFirstFactors.includes(factorId) || !Object.values(FactorIds).includes(factorId));
