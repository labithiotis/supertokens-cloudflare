...
export async function sendTryRefreshTokenResponse(recipeInstance, _, __, response, ___) {
    sendNon200ResponseWithMessage(response, "try refresh token", recipeInstance.config.sessionExpiredStatusCode);
}
export async function sendUnauthorisedResponse(recipeInstance, _, __, response, ___) {
    sendNon200ResponseWithMessage(response, "unauthorised", recipeInstance.config.sessionExpiredStatusCode);
}
export async function sendInvalidClaimResponse(recipeInstance, claimValidationErrors, __, response, ___) {
    sendNon200Response(response, recipeInstance.config.invalidClaimStatusCode, {
        message: "invalid claim",
        claimValidationErrors,
    });
}
export async function sendTokenTheftDetectedResponse(recipeInstance, sessionHandle, _, __, ___, response, userContext) {
...
export function normaliseSessionScopeOrThrowError(sessionScope) {
...
export function validateAndNormaliseUserInput(recipeInstance, appInfo, config) {
    var _a, _b, _c, _d, _e;
    ...
    return {
        ...
        jwksRefreshIntervalSec: (_e = config === null || config === void 0 ? void 0 : config.jwksRefreshIntervalSec) !== null && _e !== void 0 ? _e : 3600 * 4,
    };
}
export function normaliseSameSiteOrThrowError(sameSite) {
...
export function setAccessTokenInResponse(res, accessToken, frontToken, config, transferMethod, req, userContext) {
    setFrontTokenInHeaders(res, frontToken);
    setToken(config, res, "access", accessToken, 
    ...
    Date.now() + oneYearInMs, transferMethod, req, userContext);
    if (config.exposeAccessTokenToFrontendInCookieBasedAuth && transferMethod === "cookie") {
        ...
        // Even if the token is expired the presence of the token indicates that the user could have a valid refresh token
        // Some browsers now cap the maximum expiry at 400 days, so we set it to 1 year, which should suffice.
        Date.now() + oneYearInMs, "header", req, userContext);
    }
}
export async function getRequiredClaimValidators(session, overrideGlobalClaimValidators, userContext) {
...
export async function validateClaimsInPayload(claimValidators, newAccessTokenPayload, userContext) {
    ...
    for (const validator of claimValidators) {
        ...
        logDebugMessage("validateClaimsInPayload " + validator.id + " validation res " + JSON.stringify(claimValidationResult));
