...
export function clearSessionFromAllTokenTransferMethods(config, res, request, userContext) {
...
export function clearSession(config, res, transferMethod, request, userContext) {
...
export function getAntiCsrfTokenFromHeaders(req) {
    return req.getHeaderValue(antiCsrfHeaderKey);
}
export function setAntiCsrfTokenInHeaders(res, antiCsrfToken) {
    res.setHeader(antiCsrfHeaderKey, antiCsrfToken, false);
    res.setHeader("Access-Control-Expose-Headers", antiCsrfHeaderKey, true);
}
export function buildFrontToken(userId, atExpiry, accessTokenPayload) {
    const tokenInfo = {
        uid: userId,
        ate: atExpiry,
...
export function setFrontTokenInHeaders(res, frontToken) {
    res.setHeader(frontTokenHeaderKey, frontToken, false);
    res.setHeader("Access-Control-Expose-Headers", frontTokenHeaderKey, true);
}
export function getCORSAllowedHeaders() {
    return [antiCsrfHeaderKey, HEADER_RID, authorizationHeaderKey, authModeHeaderKey];
}
export function getCookieNameFromTokenType(tokenType) {
    switch (tokenType) {
        case "access":
...
export function getResponseHeaderNameForTokenType(tokenType) {
    switch (tokenType) {
        case "access":
...
export function getToken(req, tokenType, transferMethod) {
...
export function setToken(config, res, tokenType, value, expires, transferMethod, req, userContext) {
 ...
 * @param path
 */
export function setCookie(config, res, name, value, expires, pathType, req, userContext) {
...
export function getAuthModeFromHeader(req) {
    var _a;
 ...
 */
export function clearSessionCookiesFromOlderCookieDomain({ req, res, config, userContext, }) {
    ...
    for (const token of tokenTypes) {
        if (hasMultipleCookiesForTokenType(req, token)) {
            ...
            logDebugMessage(`clearSessionCookiesFromOlderCookieDomain: Clearing duplicate ${token} cookie with domain ${config.olderCookieDomain}`);
...
export function hasMultipleCookiesForTokenType(req, tokenType) {
