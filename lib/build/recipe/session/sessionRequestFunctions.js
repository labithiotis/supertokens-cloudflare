...
import { clearSession, clearSessionCookiesFromOlderCookieDomain, getAntiCsrfTokenFromHeaders, getAuthModeFromHeader, getToken, hasMultipleCookiesForTokenType, setCookie, } from "./cookieAndHeaders";
...
export async function getSessionFromRequest({ req, res, config, recipeInterfaceImpl, options, userContext, }) {
    ...
    for (const transferMethod of availableTokenTransferMethods) {
        ...
        if (tokenString !== undefined) {
            try {
                ...
                logDebugMessage(`getSession: ignoring token in ${transferMethod}, because it doesn't match our access token structure`);
            }
        }
    }
    ...
    if ((allowedTransferMethod === "any" || allowedTransferMethod === "header") &&
        ...
        if (hasMultipleAccessTokenCookies) {
            logDebugMessage("getSession: Throwing TRY_REFRESH_TOKEN because multiple access tokens are present in request cookies");
            throw new SessionError({
    ...
    if (session !== undefined) {
        const claimValidators = await getRequiredClaimValidators(session, options === null || options === void 0 ? void 0 : options.overrideGlobalClaimValidators, userContext);
...
*/
export async function refreshSessionInRequest({ res, req, userContext, config, recipeInterfaceImpl, }) {
    ...
    if ((allowedTransferMethod === "any" || allowedTransferMethod === "header") &&
        ...
        if ((allowedTransferMethod === "any" || allowedTransferMethod === "cookie") &&
            ...
            logDebugMessage("refreshSession: cleared all session tokens and returning UNAUTHORISED because refresh token in request is undefined");
    ...
    try {
        ...
        if (SessionError.isErrorFromSuperTokens(ex) &&
            ...
            if (req.getCookieValue(LEGACY_ID_REFRESH_TOKEN_COOKIE_NAME) !== undefined) {
                logDebugMessage("refreshSession: cleared legacy id refresh token because refresh is clearing other tokens");
                setCookie(config, res, LEGACY_ID_REFRESH_TOKEN_COOKIE_NAME, "", 0, "accessTokenPath", req, userContext);
            }
        }
        throw ex;
...
export async function createNewSessionInRequest({ req, res, userContext, recipeInstance, accessTokenPayload, userId, recipeUserId, config, appInfo, sessionDataInDatabase, tenantId, }) {
    ...
    if (outputTransferMethod === "cookie" &&
        ...
        !((appInfo.topLevelAPIDomain === "localhost" || isAnIpAddress(appInfo.topLevelAPIDomain)) &&
            (appInfo.getTopLevelWebsiteDomain({
                ...
                isAnIpAddress(appInfo.getTopLevelWebsiteDomain({
                    request: req,
                    userContext,
                }))))) {
