...
const HEADERS = new Set([
    node_buffer_1.Buffer.from(JSON.stringify({
        alg: "RS256",
        typ: "JWT",
        version: "1",
    })).toString("base64"),
...
function parseJWTWithoutSignatureVerification(jwt) {
    ...
    if (!HEADERS.has(splittedInput[0])) {
        const parsedHeader = JSON.parse(node_buffer_1.Buffer.from(splittedInput[0], "base64").toString());
    ...
    return {
        ...
        payload: JSON.parse(node_buffer_1.Buffer.from(splittedInput[1], "base64").toString()),
