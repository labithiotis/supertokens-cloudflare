...
export default class Session {
    ...
    async revokeSession(userContext) {
        ...
        if (this.reqResInfo !== undefined) {
            ...
            clearSession(this.helpers.config, this.reqResInfo.res, this.reqResInfo.transferMethod, this.reqResInfo.req, ctx);
        }
    }
    ...
    async updateSessionDataInDatabase(newSessionData, userContext) {
        ...
        if (!(await this.helpers.getRecipeImpl().updateSessionDataInDatabase({
            ...
            logDebugMessage("updateSessionDataInDatabase: Throwing UNAUTHORISED because session does not exist anymore");
            throw new STError({
                message: "Session does not exist anymore",
                type: STError.UNAUTHORISED,
            });
    ...
    async mergeIntoAccessTokenPayload(accessTokenPayloadUpdate, userContext) {
        ...
        if (response === undefined) {
            logDebugMessage("mergeIntoAccessTokenPayload: Throwing UNAUTHORISED because session does not exist anymore");
            throw new STError({
                message: "Session does not exist anymore",
                type: STError.UNAUTHORISED,
            });
        ...
        if (response.accessToken !== undefined) {
            ...
            if (this.reqResInfo !== undefined) {
                ...
                setAccessTokenInResponse(this.reqResInfo.res, this.accessToken, this.frontToken, this.helpers.config, this.reqResInfo.transferMethod, this.reqResInfo.req, ctx);
            }
        }
        else {
    ...
    attachToRequestResponse(info, userContext) {
        ...
        if (this.accessTokenUpdated) {
            ...
            const ctx = userContext === undefined && this.reqResInfo !== undefined
                ? makeDefaultUserContextFromAPI(this.reqResInfo.req)
                : getUserContext(userContext);
            setAccessTokenInResponse(res, this.accessToken, this.frontToken, this.helpers.config, transferMethod, info.req, ctx);
            if (this.refreshToken !== undefined) {
                setToken(this.helpers.config, res, "refresh", this.refreshToken.token, this.refreshToken.expiry, transferMethod, info.req, ctx);
            }
