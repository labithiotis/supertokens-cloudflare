 ...
 */
export async function createNewSession(helpers, tenantId, recipeUserId, disableAntiCsrf, accessTokenPayload, sessionDataInDatabase, userContext) {
    ...
    let response = await helpers.querier.sendPostRequest(new NormalisedURLPath(`/${tenantId}/recipe/session`), requestBody, userContext);
    return {
        session: {
 ...
 */
export async function getSession(helpers, parsedAccessToken, antiCsrfToken, doAntiCsrfCheck, alwaysCheckCore, config, userContext) {
    var _a, _b;
    let accessTokenInfo;
    try {
        /**
         ...
         */
        accessTokenInfo = await getInfoFromAccessToken(parsedAccessToken, helpers.JWKS, helpers.config.antiCsrfFunctionOrString === "VIA_TOKEN" && doAntiCsrfCheck);
    }
    catch (err) {
        /**
    ...
    if (parsedAccessToken.version >= 3) {
        ...
        if (tokenUsesDynamicKey !== helpers.config.useDynamicAccessTokenSigningKey) {
            logDebugMessage("getSession: Returning TRY_REFRESH_TOKEN because the access token doesn't match the useDynamicAccessTokenSigningKey in the config");
            throw new STError({
            ...
            if (accessTokenInfo !== undefined) {
                if (antiCsrfToken === undefined || antiCsrfToken !== accessTokenInfo.antiCsrfToken) {
                    if (antiCsrfToken === undefined) {
                        logDebugMessage("getSession: Returning TRY_REFRESH_TOKEN because antiCsrfToken is missing from request");
                        throw new STError({
                        ...
                        logDebugMessage("getSession: Returning TRY_REFRESH_TOKEN because the passed antiCsrfToken is not the same as in the access token");
                        throw new STError({
    ...
    let response = await helpers.querier.sendPostRequest(new NormalisedURLPath("/recipe/session/verify"), requestBody, userContext);
    if (response.status === "OK") {
        ...
        return Object.assign(Object.assign({}, response), { session: {
                ...
                tenantId: ((_b = response.session) === null || _b === void 0 ? void 0 : _b.tenantId) || (accessTokenInfo === null || accessTokenInfo === void 0 ? void 0 : accessTokenInfo.tenantId) || DEFAULT_TENANT_ID,
                userDataInJWT: response.session.userDataInJWT,
            } });
    }
 ...
 */
export async function getSessionInformation(helpers, sessionHandle, userContext) {
 ...
 */
export async function refreshSession(helpers, refreshToken, antiCsrfToken, disableAntiCsrf, useDynamicAccessTokenSigningKey, userContext) {
    let requestBody = {
        refreshToken,
        antiCsrfToken,
        enableAntiCsrf: !disableAntiCsrf && helpers.config.antiCsrfFunctionOrString === "VIA_TOKEN",
    ...
    let response = await helpers.querier.sendPostRequest(new NormalisedURLPath("/recipe/session/refresh"), requestBody, userContext);
    if (response.status === "OK") {
        return {
            session: {
                handle: response.session.handle,
 ...
 */
export async function revokeAllSessionsForUser(helpers, userId, revokeSessionsForLinkedAccounts, tenantId, revokeAcrossAllTenants, userContext) {
    if (tenantId === undefined) {
        tenantId = DEFAULT_TENANT_ID;
    }
    let response = await helpers.querier.sendPostRequest(new NormalisedURLPath(revokeAcrossAllTenants ? `/recipe/session/remove` : `/${tenantId}/recipe/session/remove`), {
        userId,
 ...
 */
export async function getAllSessionHandlesForUser(helpers, userId, fetchSessionsForAllLinkedAccounts, tenantId, fetchAcrossAllTenants, userContext) {
    if (tenantId === undefined) {
        tenantId = DEFAULT_TENANT_ID;
    }
    let response = await helpers.querier.sendGetRequest(new NormalisedURLPath(fetchAcrossAllTenants ? `/recipe/session/user` : `/${tenantId}/recipe/session/user`), {
        userId,
 ...
 */
export async function revokeSession(helpers, sessionHandle, userContext) {
 ...
 */
export async function revokeMultipleSessions(helpers, sessionHandles, userContext) {
    let response = await helpers.querier.sendPostRequest(new NormalisedURLPath(`/recipe/session/remove`), {
        sessionHandles,
    }, userContext);
    return response.sessionHandlesRevoked;
 ...
 */
export async function updateSessionDataInDatabase(helpers, sessionHandle, newSessionData, userContext) {
...
export async function updateAccessTokenPayload(helpers, sessionHandle, newAccessTokenPayload, userContext) {
