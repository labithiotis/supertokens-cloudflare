...
async function getSession(helpers, parsedAccessToken, antiCsrfToken, doAntiCsrfCheck, alwaysCheckCore, config, userContext) {
    ...
    try {
         ...
         */
        accessTokenInfo = await (0, accessToken_1.getInfoFromAccessToken)(parsedAccessToken, helpers.JWKS, helpers.config.antiCsrfFunctionOrString === "VIA_TOKEN" && doAntiCsrfCheck);
    }
    catch (err) {
        /**
         ...
         * error to the user
         */
        if (err instanceof error_2.default && err.type !== error_1.default.TRY_REFRESH_TOKEN) {
            throw err;
        }
        /**
