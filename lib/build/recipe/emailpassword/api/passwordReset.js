...
export default async function passwordReset(apiImplementation, tenantId, options, userContext) {
    ...
    let formFields = await validateFormFieldsOrThrowError(options.config.resetPasswordUsingTokenFeature.formFieldsForPasswordResetForm, requestBody.formFields, tenantId, userContext);
    ...
    send200Response(options.res, result.status === "OK"
        ? {
            status: "OK",
        }
        : result);
