        ...
        generatePasswordResetTokenPOST: async function ({ formFields, tenantId, options, userContext, }) {
            ...
            async function generateAndSendPasswordResetToken(primaryUserId, recipeUserId) {
                ...
                if (response.status === "UNKNOWN_USER_ID_ERROR") {
                    logDebugMessage(`Password reset email not sent, unknown user id: ${recipeUserId === undefined ? primaryUserId : recipeUserId.getAsString()}`);
                    return {
                        status: "OK",
                    };
                }
            ...
            let shouldDoAccountLinkingResponse = await AccountLinking.getInstance().config.shouldDoAutomaticAccountLinking(emailPasswordAccount !== undefined
            ...
            if (emailPasswordAccount === undefined) {
                ...
                if (!shouldDoAccountLinkingResponse.shouldAutomaticallyLink) {
                    logDebugMessage(`Password reset email not sent, since email password user didn't exist, and account linking not enabled`);
                    return {
                        status: "OK",
                    };
                }
                ...
                if (isSignUpAllowed) {
                    ...
                    logDebugMessage(`Password reset email not sent, isSignUpAllowed returned false for email: ${email}`);
                    return {
                        status: "OK",
                    };
                }
        ...
        passwordResetPOST: async function ({ formFields, token, tenantId, options, userContext, }) {
            ...
            if (existingUser.isPrimaryUser) {
                ...
                if (emailPasswordUserIsLinkedToExistingUser) {
                    return doUpdatePasswordAndVerifyEmailAndTryLinkIfNotPrimary(new RecipeUserId(userIdForWhomTokenWasGenerated));
                }
                else {
