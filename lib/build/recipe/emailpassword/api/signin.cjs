...
async function signInAPI(apiImplementation, tenantId, options, userContext) {
    ...
    // step 1
    let formFields = await (0, utils_2.validateFormFieldsOrThrowError)(options.config.signInFeature.formFields, (await options.req.getJSONBody()).formFields, tenantId, userContext);
