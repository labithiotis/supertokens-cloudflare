...
export default function getRecipeInterface(querier, getEmailPasswordConfig) {
    return {
        ...
        createNewRecipeUser: async function (input) {
            const resp = await querier.sendPostRequest(new NormalisedURLPath(`/${input.tenantId === undefined ? DEFAULT_TENANT_ID : input.tenantId}/recipe/signup`), {
                email: input.email,
        ...
        verifyCredentials: async function ({ email, password, tenantId, userContext, }) {
            const response = await querier.sendPostRequest(new NormalisedURLPath(`/${tenantId === undefined ? DEFAULT_TENANT_ID : tenantId}/recipe/signin`), {
                email,
                password,
            }, userContext);
            if (response.status === "OK") {
        ...
        createResetPasswordToken: async function ({ userId, email, tenantId, userContext, }) {
            ...
            return await querier.sendPostRequest(new NormalisedURLPath(`/${tenantId === undefined ? DEFAULT_TENANT_ID : tenantId}/recipe/user/password/reset/token`), {
                userId,
                email,
            }, userContext);
        },
        consumePasswordResetToken: async function ({ token, tenantId, userContext, }) {
            return await querier.sendPostRequest(new NormalisedURLPath(`/${tenantId === undefined ? DEFAULT_TENANT_ID : tenantId}/recipe/user/password/reset/token/consume`), {
                method: "token",
                token,
            }, userContext);
        },
