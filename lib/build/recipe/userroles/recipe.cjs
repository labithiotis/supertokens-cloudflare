  ...
  constructor(recipeId, appInfo, isInServerlessEnv, config) {
    ...
    import_postSuperTokensInitCallbacks.PostSuperTokensInitCallbacks.addPostInitCallback(() => {
      if (!this.config.skipAddingRolesToAccessToken) {
        import_recipe.default.getInstanceOrThrowError().addClaimFromOtherRecipe(import_userRoleClaim.UserRoleClaim);
      }
      if (!this.config.skipAddingPermissionsToAccessToken) {
        import_recipe.default.getInstanceOrThrowError().addClaimFromOtherRecipe(import_permissionClaim.PermissionClaim);
      }
    });
  }
  /* Init functions */
