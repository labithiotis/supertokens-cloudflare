...
export default async function signInUpAPI(apiImplementation, tenantId, options, userContext) {
    ...
    if (result.status === "OK") {
        send200Response(options.res, Object.assign({ status: result.status }, getBackwardsCompatibleUserInfo(options.req, result, userContext)));
    }
    else {
