...
export function isUsingDevelopmentClientId(client_id) {
    return client_id.startsWith(DEV_KEY_IDENTIFIER) || DEV_OAUTH_CLIENT_IDS.includes(client_id);
}
export function getActualClientIdFromDevelopmentClientId(client_id) {
...
export default function NewProvider(input) {
    var _a, _b;
    ...
    let impl = {
        ...
        exchangeAuthCodeForOAuthTokens: async function ({ redirectURIInfo }) {
            ...
            if (tokenResponse.status >= 400) {
                logDebugMessage(`Received response with status ${tokenResponse.status} and body ${tokenResponse.stringResponse}`);
        ...
        getUserInfo: async function ({ oAuthTokens, userContext }) {
            ...
            if (idToken && impl.config.jwksURI !== undefined) {
                ...
                rawUserInfoFromProvider.fromIdTokenPayload = await verifyIdTokenFromJWKSEndpointAndGetPayload(idToken, jwks, {
            ...
            if (accessToken && impl.config.userInfoEndpoint !== undefined) {
                ...
                if (userInfoFromAccessToken.status >= 400) {
                    logDebugMessage(`Received response with status ${userInfoFromAccessToken.status} and body ${userInfoFromAccessToken.stringResponse}`);
