...
function Github(input) {
    ...
    if (input.config.validateAccessToken === undefined) {
        input.config.validateAccessToken = async ({ accessToken, clientConfig }) => {
            const basicAuthToken = node_buffer_1.Buffer.from(`${clientConfig.clientId}:${clientConfig.clientSecret === undefined ? "" : clientConfig.clientSecret}`).toString("base64");
            const applicationResponse = await (0, utils_1.doPostRequest)(`https://api.github.com/applications/${clientConfig.clientId}/token`, {
    ...
    input.override = function (originalImplementation) {
        ...
        originalImplementation.getUserInfo = async function (input) {
            ...
            const rawResponse = {};
            const emailInfoResp = await (0, utils_1.doGetRequest)("https://api.github.com/user/emails", undefined, headers);
            ...
            const userInfoResp = await (0, utils_1.doGetRequest)("https://api.github.com/user", undefined, headers);
