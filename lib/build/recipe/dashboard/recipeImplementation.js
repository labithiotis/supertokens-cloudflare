    ...
    return {
        ...
        shouldAllowAccess: async function (input) {
            ...
            if (!input.config.apiKey) {
                ...
                const sessionVerificationResponse = await querier.sendPostRequest(new NormalisedURLPath("/recipe/dashboard/session/verify"), {
                ...
                if (normaliseHttpMethod(input.req.getMethod()) !== "get") {
                    ...
                    if (userEmail === undefined || typeof userEmail !== "string") {
                        logDebugMessage("User Dashboard: Returning Unauthorised because no email was returned from the core. Should never come here");
                        return false;
                    }
