...
export default async function getThirdPartyConfig(_, tenantId, options, userContext) {
    var _a, _b, _c, _d, _e;
    ...
    for (const provider of mergedProvidersFromCoreAndStatic) {
        if (provider.config.thirdPartyId === thirdPartyId) {
            ...
            for (const client of (_d = provider.config.clients) !== null && _d !== void 0 ? _d : []) {
                try {
                    const providerInstance = await findAndCreateProviderInstance(mergedProvidersFromCoreAndStatic, thirdPartyId, client.clientType, userContext);
    ...
    if (thirdPartyId.startsWith("boxy-saml")) {
        ...
        if (boxyAPIKey) {
            if (finalClients[0].clientId !== "") {
                ...
                const resp = await doGetRequest(normalisedDomain.getAsStringDangerous() +
