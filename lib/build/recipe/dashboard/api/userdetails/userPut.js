...
const updateEmailForRecipeId = async (recipeId, recipeUserId, email, tenantId, userContext) => {
    if (recipeId === "emailpassword") {
        let emailFormFields = EmailPasswordRecipe.getInstanceOrThrowError().config.signUpFeature.formFields.filter((field) => field.id === FORM_FIELD_EMAIL_ID);
...
export const userPut = async (_, tenantId, options, userContext) => {
    ...
    if (email.trim() !== "") {
        const emailUpdateResponse = await updateEmailForRecipeId(userResponse.recipe, new RecipeUserId(recipeUserId), email.trim(), tenantId, userContext);
    ...
    if (phone.trim() !== "") {
        const phoneUpdateResponse = await updatePhoneForRecipeId(new RecipeUserId(recipeUserId), phone.trim(), tenantId, userContext);
