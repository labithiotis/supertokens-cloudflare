...
export const userEmailVerifyPut = async (_, tenantId, options, userContext) => {
    ...
    if (verified) {
        const tokenResponse = await EmailVerification.createEmailVerificationToken(tenantId, new RecipeUserId(recipeUserId), undefined, userContext);
        if (tokenResponse.status === "EMAIL_ALREADY_VERIFIED_ERROR") {
            return {
                status: "OK",
            };
        }
        const verifyResponse = await EmailVerification.verifyEmailUsingToken(tenantId, tokenResponse.token, undefined, userContext);
