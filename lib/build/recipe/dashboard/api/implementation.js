    ...
    return {
        dashboardGET: async function (input) {
            ...
            if (superTokensInstance.supertokens !== undefined) {
                connectionURI =
                    new NormalisedURLDomain(superTokensInstance.supertokens.connectionURI.split(";")[0]).getAsStringDangerous() +
                        new NormalisedURLPath(superTokensInstance.supertokens.connectionURI.split(";")[0]).getAsStringDangerous();
            }
            let isSearchEnabled = false;
            const cdiVersion = await Querier.getNewInstanceOrThrowError(input.options.recipeId).getAPIVersion(input.userContext);
