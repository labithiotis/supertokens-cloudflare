    ...
    return {
        consumeCodePOST: async function (input) {
            ...
            if (preAuthChecks.status !== "OK") {
                ...
                return AuthUtils.getErrorStatusResponseWithReason(preAuthChecks, errorCodeMap, "SIGN_IN_UP_NOT_ALLOWED");
            }
            if (checkCredentialsResponseProm !== undefined) {
            ...
            if (postAuthChecks.status !== "OK") {
                return AuthUtils.getErrorStatusResponseWithReason(postAuthChecks, errorCodeMap, "SIGN_IN_UP_NOT_ALLOWED");
            }
            return {
                status: "OK",
        ...
        createCodePOST: async function (input) {
            ...
            if (input.session !== undefined) {
                ...
                if (accountInfo.email !== undefined) {
                    factorIds = factorIds.filter((factor) => [FactorIds.OTP_EMAIL, FactorIds.LINK_EMAIL].includes(factor));
                }
                else {
                    factorIds = factorIds.filter((factor) => [FactorIds.OTP_PHONE, FactorIds.LINK_PHONE].includes(factor));
                }
            }
            const preAuthChecks = await AuthUtils.preAuthChecks({
        ...
        phoneNumberExistsGET: async function (input) {
            let users = await listUsersByAccountInfo(input.tenantId, {
                phoneNumber: input.phoneNumber,
                // tenantId: input.tenantId,
        ...
        resendCodePOST: async function (input) {
            ...
            const authTypeInfo = await AuthUtils.checkAuthTypeAndLinkingStatus(input.session, {
                recipeId: "passwordless",
                email: deviceInfo.email,
                phoneNumber: deviceInfo.phoneNumber,
            ...
            while (true) {
                ...
                if (response.status === "OK") {
                    ...
                    if (input.session !== undefined) {
                        ...
                        factorIds = getEnabledPwlessFactors(input.options.config);
                        factorIds = await AuthUtils.filterOutInvalidFirstFactorsOrThrowIfAllAreInvalid(factorIds, input.tenantId, false, input.userContext);
                    }
...
async function getPasswordlessUserByAccountInfo(input) {
    ...
    logDebugMessage(`getPasswordlessUserByAccountInfo got ${existingUsers.length} from core resp ${JSON.stringify(input.accountInfo)}`);
    ...
    logDebugMessage(`getPasswordlessUserByAccountInfo ${usersWithMatchingLoginMethods.length} has matching login methods`);
