...
exports.AuthUtils = {
    ...
    preAuthChecks: async function ({ authenticatingAccountInfo, tenantId, isSignUp, isVerified, signInVerifiesLoginMethod, authenticatingUser, factorIds, skipSessionUserUpdateInCore, session, userContext, }) {
        ...
        const authTypeInfo = await exports.AuthUtils.checkAuthTypeAndLinkingStatus(session, authenticatingAccountInfo, authenticatingUser, skipSessionUserUpdateInCore, userContext);
    ...
    postAuthChecks: async function ({ authenticatedUser, recipeUserId, isSignUp, factorId, session, req, res, tenantId, userContext, }) {
        ...
        if (session !== undefined) {
            ...
            if (authenticatedUserLinkedToSessionUser) {
                ...
                (0, logger_1.logDebugMessage)(`postAuthChecks checking overwriteSessionDuringSignInUp`);
    ...
    linkToSessionIfProvidedElseCreatePrimaryUserIdOrLinkByAccountInfo: async function ({ tenantId, inputUser, recipeUserId, session, userContext, }) {
        (0, logger_1.logDebugMessage)("linkToSessionIfProvidedElseCreatePrimaryUserIdOrLinkByAccountInfo called");
        const retry = () => {
            (0, logger_1.logDebugMessage)("linkToSessionIfProvidedElseCreatePrimaryUserIdOrLinkByAccountInfo retrying....");
        ...
        const authTypeRes = await exports.AuthUtils.checkAuthTypeAndLinkingStatus(session, authLoginMethod, inputUser, false, userContext);
        if (authTypeRes.status !== "OK") {
            return authTypeRes;
        }
        if (authTypeRes.isFirstFactor) {
            if (!(0, utils_3.recipeInitDefinedShouldDoAutomaticAccountLinking)(recipe_1.default.getInstance().config)) {
                (0, logger_1.logDebugMessage)("linkToSessionIfProvidedElseCreatePrimaryUserIdOrLinkByAccountInfo skipping link by account info because this is a first factor auth and the app hasn't defined shouldDoAutomaticAccountLinking");
                return { status: "OK", user: inputUser };
            }
            (0, logger_1.logDebugMessage)("linkToSessionIfProvidedElseCreatePrimaryUserIdOrLinkByAccountInfo trying to link by account info because this is a first factor auth");
        ...
        (0, logger_1.logDebugMessage)("linkToSessionIfProvidedElseCreatePrimaryUserIdOrLinkByAccountInfo trying to link by session info");
